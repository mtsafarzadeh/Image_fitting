...
x_min=x_cat_photo_z[index_photo_z].min().astype(int)-20
x_max=x_cat_photo_z[index_photo_z].max().astype(int)+20
y_min=y_cat_photo_z[index_photo_z].min().astype(int)-20
y_max=y_cat_photo_z[index_photo_z].max().astype(int)+20
...
desired_index=(corresponding_fluxes >-4.0).nonzero()[0]
print desired_index
...
#chi2file_gt_4=open('./chi2file_gt_4_38_sources_correct.txt', 'w+')
#chi2file_gt_4=open('./chi2file_testing.txt', 'w+')
chi2file=open('./chi2file_4.5_gt_1_np_inf_65_sources_startingattrueflux_sgima0.4.txt','w+')
...
median_7sources=np.array([ 0.00367285,  0.00367554,  0.01605472,  0.01303494,  0.00846775,
        0.00431946,  0.00401625])
...
sampler = emcee.EnsembleSampler(nwalkers, ndim, digthis,threads=ndim*2)
...
np.savetxt('flat_coeff_3.5_tophat_np_inf_1.5_sources_startingattrueflux_plus_0.5_sgima0.95_500steps.txt',sampler.flatchain)
